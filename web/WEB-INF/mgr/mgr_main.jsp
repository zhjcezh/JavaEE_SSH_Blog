...
<div class="goods_list">
    ...
    <s:iterator value="list" var="article" status="st">
        <ul class="list_goods_ul">
            <li><s:property value="#st.index-4+currentPage*5"/> </li>
            <li><s:property value="#article.article_title"/></li>
            <li><s:property value="#article.category.cname"/></li>
            <li>
                <s:if test="%{ #article.article_type==1 }">公开</s:if>
                <s:if test="%{ #article.article_type==2 }">团队</s:if>
                <s:if test="%{ #article.article_type==3 }">私有</s:if>
            </li>
            <li>
...
<script>
    ...
    $("#page").paging({
        ...
        callback: function(num) {
            ...
            var id = <s:property value="#session.curUser.id"/>;
            $(window).attr('location','/article_pageList.action?currPage='+num+"&keyWord="+keyWord+"&id="+id);
        }
    });
